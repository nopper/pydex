...
pydex/src/utils $ time bzcat ~/enwiki-latest-pages-articles.xml.bz2 | python
                  wiki-extractor.py /mnt/root/collection/ 66060288
...
class WikiPageHandler(ContentHandler):
    ...
    def filter_wiki(self):
        ...
        if len(text) > ARTICLE_MIN_CHARS:
            ...
            info = tarfile.TarInfo("doc-%d-%s" % (self.num_articles,
                                                  self.title))
            info.size = len(text)

            self.file.addfile(info, StringIO.StringIO(text.encode('utf-8')))

            # Here we need to check the file size. Since we are using
            # compression a double addressing is required. This is a dirty hack
            # to get the real siz of the output file.

            if self.file.fileobj.fileobj.tell() > self.threshold:
                self.close_file()
                self.new_file()
    ...
    def new_file(self):
        handle = tempfile.NamedTemporaryFile(prefix='out-', suffix='.tgz',
                                             dir=self.output_dir, delete=False)
        self.file = tarfile.open(handle.name, mode='w:gz')
    ...
    def close_file(self):
        ...
        dst = os.path.join(self.output_dir, 'coll-{:06d}-{:06d}.tgz'.format(
                           self.num_outputs, self.collected))
        os.rename(self.file.name, dst)
        self.num_outputs += 1
        self.collected = 0

def main():
    if len(sys.argv) != 3:
        print("Usage: %s <output-dir> <bytes-soft-threshold>" % sys.argv[0])
        sys.exit(-1)
